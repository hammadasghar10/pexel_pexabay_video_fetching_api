...
class PexelsAPI:
    ...
    def fetch_health_videos(self, per_page=50):
    ...
    def save_videos(self, videos):
        ...
        insert_query = sql.SQL("INSERT INTO health (video_url) VALUES (%s) ON CONFLICT (video_url) DO NOTHING")
        for video in videos:
...
class PixabayAPI:
    ...
    def fetch_health_videos(self, per_page=50):
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to fetch health videos from Pixabay. Sorry :(", response.status_code)
            return False
...
def main():
    ...
    # Fetch and save Pexels health videos
    if pexels_api.fetch_health_videos(per_page=50):
        print("Pexels health videos successfully fetched and saved to the database.")
    else:
        print("Failed to fetch Pexels health videos.")
    
    # Fetch and save Pixabay health videos
    if pixabay_api.fetch_health_videos(per_page=50):
        print("Pixabay health videos successfully fetched and saved to the database.")
    else:
        print("Failed to fetch Pixabay health videos.")
