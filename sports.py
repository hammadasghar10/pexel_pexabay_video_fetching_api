...
class PexelsAPI:
    ...
    def fetch_sports_videos(self, per_page=50):
    ...
    def save_videos(self, videos):
        ...
        insert_query = sql.SQL("INSERT INTO sports (video_url) VALUES (%s) ON CONFLICT (video_url) DO NOTHING")
        for video in videos:
...
class PixabayAPI:
    ...
    def fetch_sports_videos(self, per_page=50):
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to fetch sports videos from Pixabay. Sorry :(", response.status_code)
            return False
...
def main():
    ...
    # Fetch and save Pexels sports videos
    if pexels_api.fetch_sports_videos(per_page=50):
        print("Pexels sports videos successfully fetched and saved to the database.")
    else:
        print("Failed to fetch Pexels sports videos.")
    
    # Fetch and save Pixabay sports videos
    if pixabay_api.fetch_sports_videos(per_page=50):
        print("Pixabay sports videos successfully fetched and saved to the database.")
    else:
        print("Failed to fetch Pixabay sports videos.")
